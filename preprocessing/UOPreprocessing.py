...
class UOPreprocessing(PreprocessingBase):
    ...
    def preprocess_collections(self):
        ...
        for index , courseblock in enumerate(courseblocks, 1):
            course_title = courseblock.find('p', attrs={'class': 'courseblocktitle'})
            course_description = courseblock.find('p', attrs={'class': 'courseblockdesc'})
            new_document = self.Document(f'CSI-{index}', course_title.text, course_description.text if course_description is not None else '')
            self.uniform_collections.append(new_document)
        
        uniform_dicts = [uniform_collection._asdict() for uniform_collection in self.uniform_collections]
        js = json.dumps(uniform_dicts)
